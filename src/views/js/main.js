...
var resizePizzas = function(size) {
    ...
    // Changed queryselector to getElementById call due to performance
...
// Also declared variable 'pizzaDiv' outside the loop so that the function makes only one DOM call
...
function updatePositions() {
    ...
    // Next for-loop will fill in the unique phase values from phaseArray into an overall movement variable until itemsLength is reached. Instead of items[i].style.left the transform & translateX option will be used due to efficiency and to reduce paint time overall.
    for (i = 0; i < numPizzas; i++) {
        //var moveX = items[i].style.left + phaseArray[(i % 5)] - 1250;
...
// Generates the sliding pizzas when the page loads. Reduced from 200 to a total of 100 pizzas calculated when scrolling
// Update: replaced the elem.basicLeft property with elem.style.left since translateX context of the transform property is used in line 528
// Also the querySelector was replaced by getElementById in order to use appendCild method. The call was additionaly taken off the for loop and saved into movingPizzas variable.
